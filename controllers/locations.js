...
const createLocation = async (req, res) => {
    ...
    if (response.acknowledged) {
        ...
        res.status(500).json(response.error || "An error occurred while creating the location.");
    }
}
...
const updateLocation = async (req, res) => {
    ...
    if (response.modifiedCount > 0) {
        ...
        res.status(500).json(response.error || "An error occurred while updating the location.");
    }
}
...
const deleteLocation = async (req, res) => {
    ...
    if (response.deletedCount > 0) {
        ...
        res.status(500).json(response.error || "An error occurred while deleting the location.");
    }
}
